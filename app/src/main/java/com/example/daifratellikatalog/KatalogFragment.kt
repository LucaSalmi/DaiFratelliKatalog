...
class KatalogFragment : Fragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val wineArray = mutableListOf<ItemsViewModel>()
        ItemsGenerator.createItems(wineArray)

        val recyclerView = view?.findViewById<RecyclerView>(R.id.wine_catalogue)
        recyclerView?.setHasFixedSize(true);
        recyclerView?.layoutManager = LinearLayoutManager(view?.context)
        val adapter = CustomAdapter(wineArray)
